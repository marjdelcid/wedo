...
export default function EditorFondos() {
  ...
  const [form, setForm] = useState({ nombre: "", descripcion: "", historia: "", meta: "", foto: "" });
  ...
  function openEdit(f: any) {
    setEditingFondo(f);
    setForm({ nombre: f.nombre || "", descripcion: f.descripcion || "", historia: f.historia || "", meta: f.meta?.toString() || "", foto: f.foto || "" });
    setShowForm(true);
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    if (editingFondo) {
      await supabase.from("fondos").update({ nombre: form.nombre, descripcion: form.descripcion, historia: form.historia, meta: parseFloat(form.meta) || 0, foto: form.foto || null }).eq("id", editingFondo.id);
    } else {
      await supabase.from("fondos").insert({ pareja_id: pareja.id, nombre: form.nombre, descripcion: form.descripcion, historia: form.historia, meta: parseFloat(form.meta) || 0, recaudado: 0, orden: fondos.length, foto: form.foto || null });
    }
    setForm({ nombre: "", descripcion: "", historia: "", meta: "", foto: "" });
