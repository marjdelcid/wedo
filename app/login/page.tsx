    ...
    <div style={{ fontFamily: "'Jost', sans-serif", background: "#FAF8F5", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ width: "100%", maxWidth: 400 }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <div style={{ fontSize: 11, color: "#A89C90", letterSpacing: 1, marginTop: 6, textTransform: "uppercase" as const }}>Bienvenida de vuelta</div>
        </div>
        <div style={{ background: "#fff", border: "1px solid rgba(26,23,20,0.08)", borderRadius: 4, padding: 32, position: "relative" as const, overflow: "hidden" }}>
          ...
          <button onClick={handleLogin} disabled={loading} style={{ width: "100%", padding: 13, background: loading ? "#A89C90" : "#8C6D4F", color: "#fff", border: "none", borderRadius: 3, fontSize: 11, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase" as const, cursor: "pointer", fontFamily: "'Jost', sans-serif" }}>
            {loading ? "Entrando..." : "Iniciar sesión"}
          </button>
        </div>
        <div style={{ textAlign: "center", marginTop: 20, fontSize: 13, color: "#A89C90" }}>
          ¿No tienes cuenta?{" "}
          <Link href="/registro" style={{ color: "#8C6D4F", fontWeight: 500, textDecoration: "none" }}>Crear mi lista gratis</Link>
        </div>
      </div>
    </div>
  );
